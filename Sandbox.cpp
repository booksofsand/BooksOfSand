...
/* MM: "DEM - Class to represent digital elevation models (DEMs) as 
       float-valued texture objects." - from DEM.h */
...
// MM: here's how to make Vrui show a dialog box
...
// MM: the following method shows how to build a main menu; might be useful
...
// MM: enclosing the following in namespace{} creates an unnamed namespace
//     AKA an anonymous namespace; makes contents only accessible from within
//     this file, avoiding repeated variable name issues. similar to static.
namespace {
...
// MM: a lot happens in the Sandbox constructor
  ...
  controlPipeFd(-1)
{
  ...
  // MM: we need camera (it's for Kinect) but do we need depth image renderer?
  //     (don't think so.) 
...
// MM: a Vrui::Application method - might be useful
...
// MM: block comment here. then does this handle detected hands or not?
#if 0
...
// MM: block comment here
  ...
  /*
    MM: following is pieces of code to print out frame buffer values during
        SARndbox process to see what the values look like. also including
	some of the printed results as example depth map summaries. 
	this does NOT print the entire depth map / frame buffer (that would be
	300,000+ values). 
