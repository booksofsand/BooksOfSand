...
void SurfaceRenderer::setAnimationTime(double newAnimationTime)
{
  ...
  fileMonitor.processEvents(); 	// MM: Vrui/IO/FileMonitor.cpp
  // MM: watches renderer's external shader source files. see SurfaceRenderer.h
}
...
void SurfaceRenderer::renderSinglePass(const int viewport[4],const PTransform& projection,const OGTransform& modelview,GLContextData& contextData) const
{
  ...
  // MM: ignore the water stuff, not useful for us
  if(waterTable!=0&&dem==0)
    {
...
// MM: NOTE THAT REMAINING CODE IN FILE IS COMMENTED OUT WITH #if 0 ... #endif
